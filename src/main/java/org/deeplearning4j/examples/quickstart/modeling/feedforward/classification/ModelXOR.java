...
public class ModelXOR {
	...
	{	
		INDArray inputA = Nd4j.zeros(1, 2);
        	inputA.putScalar(new int[]{0, 0}, 0);
        	inputA.putScalar(new int[]{0, 1}, 0);
		INDArray outputA = net.output(inputA);
		log.info("inputA = " + inputA);
	        log.info("outputA = " + outputA);
	}
	...
	writeModelToFile(net, "/root/data/xor-nn-model-2.zip");
